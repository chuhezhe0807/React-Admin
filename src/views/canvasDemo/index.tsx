...
const CanvasDemo = () => {
  ...
  const drawLineGrid = (scaleVal = scale) => {
    ...
    for (let item of canvasObjRef.current.objects) {
      switch (item.type) {
        case "TRIANGLE":
          temp = new Triangle(ctxVal, item.x, item.y)
          break
        default:
          break
      }
  ...
  const mousewheel = (e: any) => {
    ...
    e.preventDefault()
    if (scale <= CoordinateSystem.MIN_SCALESIZE) {
      setScale(scale + CoordinateSystem.SCALE_ABILITY)
      drawLineGrid()
      return
    }
    if (scale >= CoordinateSystem.MAX_SCALESIZE) {
    ...
    function getClickPoint(e: any) {
      switch (type) {
        case BlockType.TRIANGLE:
          ...
          createBlock = new Triangle(ctxVal, e.detail.offsetX, e.detail.offsetY)
  ...
  return (
    <div className="canvas" style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ width: "1000px", border: "1px solid #999", boxSizing: "border-box" }}>
